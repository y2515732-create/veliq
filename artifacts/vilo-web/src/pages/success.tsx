...
export default function Success() {
  ...
  const { data: user, isLoading } = useGetUser(email, {
    query: {
      enabled: !!email,
      queryKey: getGetUserQueryKey(email),
      refetchInterval: (data) => (data?.state?.data?.status !== "active" ? 3000 : false),
    },
  });
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6">
        <div className="max-w-md text-center">
          ...
          <p className="text-muted-foreground mb-8">No email provided in the URL.</p>
          <Link href="/">
  ...
  const isPending = !user || user.status === "pending" || isLoading;
